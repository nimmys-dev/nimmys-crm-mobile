...
class LeadsCubit extends BaseCubit<LeadsState> {
  ...
  Future<void> goToLeadsPage(int page) async {
    if (page < 1 ||
        page == state.leadPagination?.currentPage) {
      return;
    }
  ...
  Future<void> _fetchLeadsPage({
    ...
    final String query =
        (search ?? state.leadSearchQuery).trim();
    ...
    if (!searchChanged &&
        ...
        page ==
            (state.leadPagination?.currentPage ?? 1)) {
      return;
    }
    ...
    emit(
      state.copyWith(
        ...
        leadList: searchChanged
            ? <LeadItemData>[]
            : state.leadList,
      ),
    );

    final Result<LeadListResponse> result =
  ...
  void _setLeadAssigneesUIState(
    UIState<LeadAssigneeSuccess>? uiState,
  ) {
    emit(
      state.copyWith(
        leadAssigneesUIState: uiState,
      ),
    );
  }

  Future<void> getLeadAssignees({
    bool force = false,
  }) async {
    if (!force &&
        state.leadAssigneesUIState?.data != null) {
      return;
    }

    _setLeadAssigneesUIState(
      UIState.loading(),
    );
  ...
  void _setLeadSourcesUIState(
    UIState<LeadSourceModel>? uiState,
  ) {
    emit(
      state.copyWith(
        leadSourcesUIState: uiState,
      ),
    );
  }

  Future<void> getLeadSources({
    bool force = false,
  }) async {
    if (!force &&
        state.leadSourcesUIState?.data != null) {
      return;
    }
    ...
    final Result<LeadSourceModel> result =
        await _repository.getLeadSources();
  ...
  void resetLeadSourcesState() {
    _setLeadSourcesUIState(
      resetUIState<LeadSourceModel>(
        state.leadSourcesUIState,
      ),
    );
  }
  ...
  void _setCreateLeadUIState(
    UIState<dynamic>? uiState,
  ) {
    emit(
      state.copyWith(
        createLeadUIState: uiState,
      ),
    );
  }
  ...
  Future<void> createLead(
    ...
    if (state.createLeadUIState?.status ==
        Status.LOADING) {
      return;
    }
    ...
    final Result<dynamic> result =
        await _repository.createLead(payload);
  ...
  void _setLeadDetailsUIState(
    UIState<LeadDetailsSuccess>? uiState,
  ) {
    emit(
      state.copyWith(
        leadDetailsUIState: uiState,
      ),
    );
  }
  ...
  void resetLeadDetailsState() {
    _setLeadDetailsUIState(
      resetUIState<LeadDetailsSuccess>(
        state.leadDetailsUIState,
      ),
    );
  }
  ...
  void _setUpdateLeadUIState(
    UIState<LeadDetailsSuccess>? uiState,
  ) {
    emit(
      state.copyWith(
        updateLeadUIState: uiState,
      ),
    );
  }
  ...
  Future<void> updateLead(
    ...
    if (state.updateLeadUIState?.status ==
        Status.LOADING) {
      return;
    }
  ...
  void _setQuotationPdfUIState(
    UIState<QuotationPdfResponse>? uiState,
  ) {
    emit(
      state.copyWith(
        quotationPdfUIState: uiState,
      ),
    );
  }
  ...
  Future<void> getQuotationPdf(int leadId) async {
    ...
    final Result<QuotationPdfResponse> result =
        await _repository.getQuotationPdf(
      leadId,
    );
  ...
  void resetQuotationPdfState() {
    _setQuotationPdfUIState(
      resetUIState<QuotationPdfResponse>(
        state.quotationPdfUIState,
      ),
    );
  }
  ...
  void _setAddCallLogUIState(
    UIState<TeleCallDetailResponseModel>? uiState,
  ) {
    emit(
      state.copyWith(
        addCallLogUIState: uiState,
      ),
    );
  }
  ...
  Future<void> addCallLog({
    ...
    if (state.addCallLogUIState?.status ==
        Status.LOADING) {
      return;
    }
    ...
    final Result<TeleCallDetailResponseModel> result =
        await _repository.addCallLog(
      leadId: leadId,
    ...
    if (result
        is Success<TeleCallDetailResponseModel>) {
      _setAddCallLogUIState(
        UIState.success(result.value),
      );
      ...
      unawaited(
        ...
        is Error<TeleCallDetailResponseModel>) {
      _setAddCallLogUIState(
        UIState.error(result.type),
      );
    }
