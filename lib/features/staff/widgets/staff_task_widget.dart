...
class _StaffTasksSectionState extends State<StaffTasksSection> {
  ...
  Future<void> _selectAllTasks() async {
    ...
    try {
      ...
      // Select every task from the accumulated list.
      final Set<int> allIds = cubit.state.myTasksList
  ...
  Future<void> _goToPreviousPage() async {
    final StaffCubit cubit = context.read<StaffCubit>();
    final pagination = cubit.state.myTasksPagination;
    ...
    // Page navigation is independent from load-more.
    // It fetches the exact previous page and replaces the visible list.
    await cubit.goToMyTasksPage(currentPage - 1);
  ...
  Future<void> _goToNextPage() async {
    ...
    // Page navigation fetches the exact page and replaces the visible list.
    await cubit.goToMyTasksPage(nextPage);
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<StaffCubit, StaffState>(
      ...
      builder: (context, state) {
        ...
        // Clear selection when navigating to another page/search result.
        if (_selectedTaskIds.isNotEmpty &&
            _selectedTaskIds.any((id) => !tasks.any((task) => task.id == id))) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted) {
              setState(() {
                _selectedTaskIds.clear();
        ...
        return AppSectionCard(
          child: Column(
            ...
            children: [
              ...
              // IMPORTANT:
              // No ListView here.
              // The parent screen already provides scrolling.
              // -----------------------------------------------------------------
              if (isLoading && tasks.isEmpty)
                const Padding(
                  padding: EdgeInsets.symmetric(vertical: 16),
                ...
                Column(
                  ...
                  children: tasks.map((task) {
                    ...
                    final bool isSelected = _selectedTaskIds.contains(id);
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: _selectedStaff == null
                      ? null
                      : () {
                          final int? targetId = _selectedStaff!.id;
